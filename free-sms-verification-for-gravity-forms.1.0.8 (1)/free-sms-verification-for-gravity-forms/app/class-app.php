...
class App {
	...
	public function register_hooks() {
		add_action( 'admin_init', array($this,'require_gf'));
		...
		add_action( 'wp_print_scripts', array($this,'check_if_firebase_exist'), 100 );
	}

	

	function check_if_firebase_exist() {
		if(wp_script_is(GF_FREE_SMS_VERIFICATION . 'firebase_app')){
			if(wp_script_is('firebase')){
				wp_dequeue_script('firebase');
				wp_deregister_script('firebase');
			}
	
			if(wp_script_is('firebase-auth')){
