...
class GF_SMS_Addon extends \GFAddOn {
	...
	public function enq_styles_scripts( $form, $is_ajax ) {
		...
		foreach ( $form['fields'] as $key => $field ) {
			if ( 'gf-free-sms-verification' === $field['type'] ) {
				...
				$firebase_default_country = $field['firebase_default_country'];
				$firebase_recpatcha_type = $field['firebase_recpatcha_type'];
				$exist              = true;
			}
		}
		...
		$translation_array = array(
			...
			'firebase_default_country' => $firebase_default_country,
			'firebase_recpatcha_type'=>$firebase_recpatcha_type,
		);
	...
	public function tooltips( $tooltips ) {
		$simple_tooltips = array(
			'firebase_countries' => esc_html__( 'Choose the countries that will show (Leave empty to show all)', 'gf-free-sms-verification' ),
		);
	...
	public function my_standard_settings( $position, $form_id ) {
		if ( 250 === $position ) {
			...
			<li class="firebase_default_country field_setting">
				<label for="firebase_default_country" class="section_label">
					<?php esc_html_e( 'Firebase Default Country', 'gf-free-sms-verification' ); ?>
					<?php gform_tooltip( 'firebase_default_country' ); ?>
				</label>
				<select class="gf_sms_select2" style="width:100%;" id="firebase_default_country" onkeyup="setDefaultCountry(jQuery(this).val());" onchange="setDefaultCountry(jQuery(this).val());">
					<?php
					foreach ( $this->get_whitelisted_countries() as $key => $val ) {
						echo '<option value="' . esc_attr( $key ) . '">' . esc_attr( $val ) . '</option>';
					}
			...
			<li class="firebase_recpatcha_type field_setting">
				<label for="firebase_recpatcha_type" class="section_label">
					<?php esc_html_e( 'Firebase Recpatcha Type', 'gf-free-sms-verification' ); ?>
					<?php gform_tooltip( 'firebase_recpatcha_type' ); ?>
				</label>
				<select style="width:100%;" id="firebase_recpatcha_type" onkeyup="setRecaptchaType(jQuery(this).val());" onchange="setRecaptchaType(jQuery(this).val());">
					<option value="rec1">Recaptcha v2</option>
					<option value="rec2">Invisible Recaptcha</option>
				</select>
			</li>
